...
//            Downloads.COLUMN_FILE_NAME_HINT, Downloads._DATA, };
    ...
    public static  final String[] UNDERLYING_COLUMNS={
            Downloads._ID, Downloads.COLUMN_TITLE,
            Downloads.COLUMN_NO_INTEGRITY,Downloads.COLUMN_VISIBILITY,
            //
            Constants.FAILED_CONNECTIONS,Constants.RETRY_AFTER_X_REDIRECT_COUNT,
            Downloads.COLUMN_NOTIFICATION_PACKAGE,Downloads.COLUMN_NOTIFICATION_CLASS,
            Downloads.COLUMN_USER_AGENT,Downloads.COLUMN_REFERER,Constants.ETAG,
            Downloads.COLUMN_DELETED,Downloads.COLUMN_IS_PUBLIC_API,
            Downloads.COLUMN_ALLOW_ROAMING,Downloads.COLUMN_BYPASS_RECOMMENDED_SIZE_LIMIT,
            Downloads.COLUMN_NOTIFICATION_EXTRAS,Downloads.COLUMN_COOKIE_DATA,
            Downloads.COLUMN_CONTROL,Downloads.COLUMN_ALLOWED_NETWORK_TYPES,
            Downloads.COLUMN_FILE_UID,
            //
            Downloads.COLUMN_DESCRIPTION, Downloads.COLUMN_URI,
            Downloads.COLUMN_MIME_TYPE, Downloads.COLUMN_TOTAL_BYTES,
            Downloads.COLUMN_STATUS, Downloads.COLUMN_CURRENT_BYTES,
    ...
    public static class Request {
        ...
        public Request setUid(CharSequence uid) {
            mUid = uid;
            return this;
        }
        /**
        ...
        ContentValues toContentValues(String packageName) {
            ...
            putIfNonNull(values, Downloads.COLUMN_FILE_UID, mUid);
    ...
    public static class Query {
        ...
        Cursor runQuery(ContentResolver resolver, String[] projection,
            ...
            if(mMimeType!=null){
                selectionParts.add(Downloads.COLUMN_MIME_TYPE
                        + "= '"+mMimeType+"'");
            }
            if(mUri!=null){
                selectionParts.add(Downloads.COLUMN_URI
                        + "= '"+mUri+"'");
            }
            if(mUid!=null){
                selectionParts.add(Downloads.COLUMN_FILE_UID
                        + "= '"+mUid+"'");
            }
        ...
        public void setFilterByMimeType(String mimeType) {
            this.mMimeType=mimeType;
        }
        public void setFilterByUid(String uid) {
            this.mUid=uid;
        }
        public void setFilterByUri(String uri) {
            this.mUri=uri;
        }
    }
     ...
     */
    private Cursor queryCursorTranslator(Query query) {
        Cursor underlyingCursor =query(query);
    ...
    public void pauseDownload(long... ids) {
        Cursor cursor = queryCursorTranslator(new Query().setFilterById(ids));
        try {
     ...
     * 获取下载任务<br>
     */
    public  HashMap<String, DownloadInfo> queryAll(String mimeType) {
...
//        query.r
        ...
        //类型过滤
        query.setFilterByMimeType(mimeType);
        //id过滤
...
//        List<DownloadInfo> mDownloads = new ArrayList<>();
        for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
            DownloadInfo.Reader reader = new DownloadInfo.Reader(mResolver, cursor);
...
//            mDownloads.add(info);
    ...
    public  DownloadInfo query(long id) {
        DownloadManager manager = Session.getInstance().getDownloadManager();
        DownloadManager.Query query = new DownloadManager.Query();
        query.setFilterById(id);
        Cursor cursor = manager.query(query);
...
//            mDownloads.add(info);
    ...
    public  DownloadInfo query(String uid) {
        DownloadManager manager = Session.getInstance().getDownloadManager();
        DownloadManager.Query query = new DownloadManager.Query();
...
//            mDownloads.add(info);
    ...
    public  DownloadInfo query(Uri uri) {
        DownloadManager manager = Session.getInstance().getDownloadManager();
        DownloadManager.Query query = new DownloadManager.Query();
        query.setFilterByUri(uri.toString());
        Cursor cursor = manager.query(query);
        if (cursor.moveToFirst()) {
            DownloadInfo.Reader reader = new DownloadInfo.Reader(mResolver, cursor);
            DownloadInfo info = reader.newDownloadInfo(BaseApplication.getInstance());
