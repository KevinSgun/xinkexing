...
class CircularProgressBarUtils {
  ...
  static void checkAngle(int angle) {
    if (angle < 0 || angle > 360)
      throw new IllegalArgumentException(String.format(Locale.CHINESE,"Illegal angle %d: must be >=0 and <= 360", angle));
  }

  static void checkPositiveOrZero(float number, String name) {
    if (number < 0)
      throw new IllegalArgumentException(String.format(Locale.CHINESE,"%s %d must be positive", name, (int)number));
  }
