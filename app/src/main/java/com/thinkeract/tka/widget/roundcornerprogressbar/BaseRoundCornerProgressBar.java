...
package com.thinkeract.tka.widget.roundcornerprogressbar;
...
/**
 * Created by ymh on 11/19/17 AD 21:30
...
public abstract class BaseRoundCornerProgressBar extends LinearLayout {
    ...
    protected final static int DEFAULT_PROGRESS_RADIUS = 5;
    ...
    protected abstract void drawProgress(LinearLayout layoutProgress, float max, float progress, float totalLength,
    ...
    public void setupStyleable(Context context, AttributeSet attrs) {
        ...
        isOrientationVertical = typedArray.getBoolean(R.styleable.RoundCornerProgress_rcOrientationVertical,false);
        typedArray.recycle();
    ...
    protected void onSizeChanged(int newWidth, int newHeight, int oldWidth, int oldHeight) {
        ...
        if(!isInEditMode()) {
            if(isOrientationVertical){
                totalLength = newHeight;
            }else{
                totalLength = newWidth;
            }
    ...
    private void drawPrimaryProgress() {
        drawProgress(layoutProgress, max, progress, totalLength, radius, padding, colorProgress, isReverse);
    }

    private void drawSecondaryProgress() {
        drawProgress(layoutSecondaryProgress, max, secondaryProgress, totalLength, radius, padding, colorSecondaryProgress, isReverse);
    }
    ...
    private void setupReverse(LinearLayout layoutProgress) {
        ...
        if (isReverse) {
            progressParams.addRule(isOrientationVertical? RelativeLayout.ALIGN_PARENT_BOTTOM: RelativeLayout.ALIGN_PARENT_RIGHT);
            // For support with RTL on API 17 or more
            if (!isOrientationVertical&& Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
                progressParams.addRule(RelativeLayout.ALIGN_PARENT_END);
        } else {
            progressParams.addRule(isOrientationVertical? RelativeLayout.ALIGN_PARENT_TOP: RelativeLayout.ALIGN_PARENT_LEFT);
            // For support with RTL on API 17 or more
            if (!isOrientationVertical&& Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1)
