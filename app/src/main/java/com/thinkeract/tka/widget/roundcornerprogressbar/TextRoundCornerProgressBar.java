...
/**
 * Created by ymh on 11/19/17 AD 21:32
...
public class TextRoundCornerProgressBar extends BaseRoundCornerProgressBar implements ViewTreeObserver.OnGlobalLayoutListener {
    protected final static int DEFAULT_TEXT_SIZE = 11;
    ...
    @Override
    protected void drawProgress(LinearLayout layoutProgress, float max, float progress, float totalLength,
        ...
        int progressLength = (int) ((totalLength - (padding * 2)) / ratio);
        ViewGroup.LayoutParams progressParams = layoutProgress.getLayoutParams();
        if (isOrientationVertical)
            progressParams.height = progressLength;
        else
            progressParams.width = progressLength;
    ...
    private void drawTextProgressMargin() {
        ...
        if(!isOrientationVertical) {
            ...
            params.setMargins(0, textProgressMargin, 0, textProgressMargin);
        }
    ...
    private void alignTextProgressOutsideProgress() {
        ...
        if (isReverse()) {
            if (!isOrientationVertical) {
                ...
                params.addRule(RelativeLayout.ABOVE, R.id.layout_progress);
                params.addRule(RelativeLayout.CENTER_HORIZONTAL,R.id.layout_progress_holder);
            }
        } else {
            if (!isOrientationVertical) {
                ...
                params.addRule(RelativeLayout.BELOW, R.id.layout_progress);
